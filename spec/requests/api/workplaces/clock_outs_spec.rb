...
RSpec.describe "POST /api/workplaces/:id/clock_outs", type: :request do
  context "when API key is not given" do
    it "results error" do
      post "/api/workplaces/1/clock_outs"
  ...
  context "when API key is given" do
    ...
    it "creates clock-in record" do
      expect {
        post "/api/workplaces/#{workplace.id}/clock_outs", headers: request_headers
      }.to change { workplace.attendance_records.count }.by(1)

      expect(response).to be_created

      attendance_record = workplace.attendance_records.last
      expect(attendance_record.event).to eq "clock-out"
    end
  end
end
